...
def importData(path):
    ...
    inputfile = open(pathtofile, 'r') # open file in "read" mode
    data = inputfile.readlines() # make list of all lines in file
    inputfile.close() # close file after reading it
...
# writes a result file in given test directory named {algname}-result.csv formatted as {available frames};{page faults}
def exportData(path, algname, result):

    pathtofile = os.path.join(path, f'{algname}-result.csv') # append filename to test directory path
    ...
    resultfile.close() # close file after writing
    return
